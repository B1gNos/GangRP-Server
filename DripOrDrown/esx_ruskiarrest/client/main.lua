...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(0, Keys['LEFTSHIFT']) and IsControlPressed(0, Keys['Z']) and not Aresztuje and GetGameTimer() - OstatnioAresztowany > 10 * 1000 and PlayerData.job.name == PracaPolice then	-- Mozesz tutaj zmienic przyciski
			Citizen.Wait(10)
			...
			if distance ~= -1 and distance <= Config.ArrestDistance and not Aresztuje and not Aresztowany and not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				...
				ESX.ShowNotification("~r~You are arresting a citizen ~r~ " .. GetPlayerServerId(closestPlayer) .. "")						-- Drukuje Notyfikacje
				...
				ESX.ShowNotification("~g~Citizen arrested ~r~ " .. GetPlayerServerId(closestPlayer) .. "")					-- Drukuje Notyfikacje
