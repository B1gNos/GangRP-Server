...
Citizen.CreateThread(function()
	...
	mainMenu = NativeUI.CreateMenu("~w~Drip or Drown", "~w~Staff Menu", menuOrientation, 0)
...
function GenerateMenu() -- this is a big ass function
	...
	mainMenu = NativeUI.CreateMenu("~w~Drip or Drown", "~w~Admin Menu", menuOrientation, 0)
	_menuPool:Add(mainMenu)
	...
	for i,thePlayer in ipairs(players) do
		...
		thisItem.Activated = function(ParentMenu, SelectedItem)
			...
			if result ~= '' then
				TriggerServerEvent("EasyAdmin:SendMessage", GetPlayerServerId(thePlayer), result)
			end
		end
		
		local thisItem = NativeUI.CreateItem("~y~Revive", "~y~Revive Player")
		thisPlayer:AddItem(thisItem)
		thisItem.Activated = function(ParentMenu,SelectedItem)
			TriggerServerEvent("esx_ambulancejob:revive", GetPlayerServerId(thePlayer))
		end

		local thisItem = NativeUI.CreateItem("~y~Force Skin", "Make them change skin")
		thisPlayer:AddItem(thisItem)
		thisItem.Activated = function(ParentMenu,SelectedItem)
			TriggerServerEvent("EasyAdmin:ForceSkin", GetPlayerServerId(thePlayer))
		end
