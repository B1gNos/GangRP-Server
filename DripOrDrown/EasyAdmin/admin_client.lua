...
RegisterNetEvent("EasyAdmin:Notify")
AddEventHandler('EasyAdmin:Notify', function(message)
...
AddEventHandler('EasyAdmin:SendMessage', function(message)
	CreateThread(function()
		...
		while displayTime ~= 0 do
			...
			PushScaleformMovieFunctionParameterString("~b~Message From Staff")
...
RegisterNetEvent("EasyAdmin:SetPed")
AddEventHandler('EasyAdmin:SetPed', function(ped)
	local _source = source
	local pedType = ped

	CreateThread(function()
		pedType = GetHashKey(pedType)

		if IsModelValid(pedType) then
			...
			SetPlayerModel(PlayerId(), pedType)
