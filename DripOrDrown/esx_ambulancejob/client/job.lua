...
function OpenAmbulanceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'ambulance_actions',
	{
		...
		if data.current.value == 'cloakroom' then
			OpenCloakroomMenu()
        elseif data.current.value == 'pharmacy' then
            OpenPharmacyMenu()
							...
							if quantity > 0 then
								...
								if IsPedDeadOrDying(closestPlayerPed, 1) then
									...
									if Config.ReviveReward > 0 then
										ESX.ShowNotification(_U('revive_complete_award', GetPlayerName(closestPlayer), Config.ReviveReward), "success")
									else
										ESX.ShowNotification(_U('revive_complete', GetPlayerName(closestPlayer)), "success")
									end
								else
									ESX.ShowNotification(_U('player_not_unconscious'), "error")
								end
							else
								ESX.ShowNotification(_U('not_enough_medikit'), "error")
							end
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									IsBusy = true
									ESX.ShowNotification(_U('heal_inprogress'), "success")
									...
									ESX.ShowNotification(_U('heal_complete', GetPlayerName(closestPlayer)), "success")
									IsBusy = false
								else
									ESX.ShowNotification(_U('player_not_conscious'), "error")
								end
							else
								ESX.ShowNotification(_U('not_enough_bandage'), "error")
							end
						end, 'bandage')
		...
		elseif data.current.action == 'garage' then
			...
			ESX.TriggerServerCallback('esx_vehicleshop:retrieveJobVehicles', function(jobVehicles)
				if #jobVehicles > 0 then
						...
						if data2.current.stored then
							...
							if foundSpawn then
								...
								ESX.Game.SpawnVehicle(data2.current.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
									...
									ESX.ShowNotification(_U('garage_released'), "success")
								end)
							end
						else
							ESX.ShowNotification(_U('garage_notavailable'), "error")
						end
					...
					ESX.ShowNotification(_U('garage_empty'), "error")
				end
			end, 'car')
...
function StoreNearbyVehicle(playerCoords)
	...
	if #vehicles > 0 then
		...
		ESX.ShowNotification(_U('garage_store_nearby'), "error")
		return
	end
	...
	ESX.TriggerServerCallback('esx_ambulancejob:storeNearbyVehicle', function(storeSuccess, foundNum)
		if storeSuccess then
			...
			IsBusy = false
			ESX.ShowNotification(_U('garage_has_stored'), "success")
		else
			ESX.ShowNotification(_U('garage_has_notstored'), "error")
		end
	end, vehiclePlates)
end
...
function GetAvailableVehicleSpawnPoint(hospital, part, partNum)
	...
	if found then
		...
		ESX.ShowNotification(_U('garage_blocked'), "error")
		return false
	end
end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'helicopter_spawner', {
		...
		if data.current.action == 'buy_helicopter' then
			...
			if #authorizedHelicopters > 0 then
				...
				ESX.ShowNotification(_U('helicopter_notauthorized'), "error")
				return
			end
			...
			ESX.TriggerServerCallback('esx_vehicleshop:retrieveJobVehicles', function(jobVehicles)
				if #jobVehicles > 0 then
					for k,v in ipairs(jobVehicles) do
						local props = json.decode(v.vehicle)
						local vehicleName = GetLabelText(GetDisplayNameFromVehicleModel(props.model))
						local label = ('%s - <span style="color:white;">%s</span>: '):format(vehicleName, props.plate)
